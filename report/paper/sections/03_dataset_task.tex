...
The study should therefore be read as a controlled T2-weighted signal-representation experiment, not as a substitute for biparametric or multiparametric clinical prostate MRI.
...
After alignment, let $\mathbf{x}_{\mathrm{aligned}}$ denote the resulting four-coil complex image. The representation is then selected before scaling:
\begin{align}
\mathbf{u}_{\mathrm{img}} &= \mathbf{x}_{\mathrm{aligned}},\\
\mathbf{u}_{F} &= \mathcal{F}_{c}\{\mathbf{x}_{\mathrm{aligned}}\},
\end{align}
where $\mathcal{F}_{c}$ is the centered orthonormal two-dimensional FFT applied independently to each coil. For each selected domain $d\in\{\mathrm{img},F\}$, a separate sample-level scalar is computed from the complex-magnitude elements of that representation,
\begin{equation}
q_d=P_{99}\!\left(|\mathbf{u}_d|\right),\qquad
\widetilde{\mathbf{u}}_d=\frac{\mathbf{u}_d}{q_d+\epsilon}.
\label{eq:representation-scale}
\end{equation}
The percentile is computed over all four coils and spatial locations and the resulting scalar is shared across those coils. Thus the image-domain and Fourier-domain representations have separate robust scaling scalars, each applied after its own representation has been selected.

\subsection{Image-Domain and Fourier-Domain Inputs}
The factorial grid includes two complex input domains. The image-domain branch receives $\widetilde{\mathbf{u}}_{\mathrm{img}}$, whereas the Fourier-domain branch receives $\widetilde{\mathbf{u}}_{F}$. Learning in image and Fourier coordinates has been studied extensively in MRI reconstruction; cross-domain designs such as KIKI-net explicitly alternate image- and k-space CNNs~\cite{Eo2018KIKINet}, and more recent work has examined how spatial versus k-space input changes the inductive bias of MRI segmentation networks~\cite{Gosche2024DomainInfluence}. In the present study the domain factor is deliberately simpler: the aligned complex image is presented either in image coordinates or as an FFT-derived Fourier-domain representation, without a reconstruction/data-consistency loop. This experimental input is therefore an FFT-derived Fourier-domain representation of the aligned complex images, not untouched scanner raw k-space.

The real baseline receives
\begin{equation}
x_{\mathrm{real}}=|\widetilde{\mathbf{u}}_{\mathrm{img}}|\in\mathbb{R}^{4\times H\times W}.
\label{eq:real-input}
\end{equation}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{../figures/figure_input_representation/figure1_artwork.pdf}
\caption{Representative prepared T2-weighted slice shown for the four selected coil channels as image-domain magnitude, image-domain phase, Fourier-domain log magnitude, and Fourier-domain phase.}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{../figures/figure_preprocessing_pipeline/figure2_artwork.pdf}
\caption{Preprocessing pipeline for the prepared four-coil T2-weighted input. Phase alignment is followed by image-domain or centered-FFT representation selection and robust magnitude scaling using a sample-level scalar shared across coils within each representation. The real baseline uses image-domain magnitude.}
\label{fig:pipeline}
\end{figure*}
