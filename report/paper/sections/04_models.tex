...
The complex-only model uses four complex blocks, each containing two complex convolutions, normalization, and activation.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{../figures/figure_interactions/figure5_artwork.pdf}
\caption{Implemented interaction mechanisms. (a) The modulus cross-stream gate derives a real-valued gate from complex-feature magnitude and applies it to the real branch. (b) Holographic attention operates within the complex branch using complex-valued query, key, and value projections with an additional magnitude-distance term.}
\label{fig:interactions}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{../figures/figure_model_families/figure3_artwork.pdf}
\caption{High-level architectures of the real-valued baseline, complex-only network, and dual-stream network. The first three stages combine a feature block and pooling operation; the dual-stream architecture processes real magnitude and complex features in parallel before feature concatenation and classification.}
